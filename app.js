const Manager = require("./10-teamGenerator/lib/Manager");
const Engineer = require("./10-teamGenerator/lib/Engineer");
const Intern = require("./10-teamGenerator/lib/Intern");
...
const render = require("./10-teamGenerator/lib/htmlRenderer");
...
function buildHtmlPage() {
    let newFile = fs.readFileSync("./10-teamGenerator/templates/main.html")
    fs.writeFileSync("./10-teamGenerator/teamPage.html", newFile, function (err) {
        if (err) throw err;
    })
    ...
    fs.appendFileSync("./10-teamGenerator/teamPage.html", "</div></main></body></html>", function (err) {
        if (err) throw err;
    });
...
function buildHtmlCard(memberType, name, id, email, propertyValue) {
    let data = fs.readFileSync(`./10-teamGenerator/templates/${memberType}.html`, 'utf8')
    ...
    fs.appendFileSync("./10-teamGenerator/teamPage.html", data, err => { if (err) throw err; })
